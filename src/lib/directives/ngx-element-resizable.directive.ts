...
@Directive({
    selector: '[ngxElementResizable]'
})
export class NgxElementResizableDirective implements OnInit {
    @Input()
    offset = 8;
    ...
    constructor(private element: ElementRef,
                ...
                private ngxElemntResizableService: NgxElemntResizableService) {
    }
    ...
    private styleValue(styleName: string) {
        const pxValue = this.element.nativeElement.style[this.ngxElemntResizableService.tranformStyle(styleName)];
        const value = this.ngxElemntResizableService.getPxValue(pxValue);
        this.renderer.setStyle(this.element.nativeElement, styleName, '');
        if (value) {
            this[this.ngxElemntResizableService.tranformStyle(styleName)] = value;
        }
    }
    ...
    mousemove(event) {
        ...
        dir1 = this.ngxElemntResizableService.getCursorDir(this.element, dir1);
        ...
        if (~this.dir.indexOf('e') || ~this.dir.indexOf('w')) {
            let offsetX = event.clientX - this.client.clientX;
            // 计算规则是否相反 为n的时候是算法是反的为-1
            const ver = ~this.dir.indexOf('w') ? -1 : 1;
            ...
            if (this.minWidth && this.minWidth > changedWidth) {
                ...
                newClient.clientX = ~this.dir.indexOf('w') ? left + width - this.minWidth : left + this.minWidth;
            } else if (this.maxWidth && this.maxWidth < changedWidth) {
                changedWidth = this.maxWidth;
                offsetX = ver * (this.maxWidth - el.offsetWidth);
                const {left, width} = this.element.nativeElement.getBoundingClientRect();
                newClient.clientX = ~this.dir.indexOf('w') ? left + width - this.maxWidth : left + this.maxWidth;
            }
            this.renderer.setStyle(this.element.nativeElement, 'width', `${changedWidth}px`);
            if (~this.dir.indexOf('w') && isAbsolute) {
                const left = el.offsetLeft - this.ngxElemntResizableService.getPxValue(el.style.marginLeft) + offsetX;
                this.renderer.setStyle(el, 'left', `${left}px`);
            }
        }
        if (~this.dir.indexOf('n') || ~this.dir.indexOf('s')) {
            let offsetY = event.clientY - this.client.clientY;
            // 计算规则是否相反 为n的时候是算法是反的为-1
            const ver = ~this.dir.indexOf('n') ? -1 : 1;
            ...
            if (this.minHeight && this.minHeight > changedHeight) {
                ...
                newClient.clientY = ~this.dir.indexOf('n') ? top + height - this.minHeight : top + this.minHeight;
                ...
                const {top, height} = this.element.nativeElement.getBoundingClientRect();
                newClient.clientY = ~this.dir.indexOf('n') ? top + height - this.maxHeight : top + this.maxHeight;
            }
            this.renderer.setStyle(this.element.nativeElement, 'height', `${changedHeight}px`);
            if (~this.dir.indexOf('n') && isAbsolute) {
                const top = el.offsetTop - this.ngxElemntResizableService.getPxValue(el.style.marginTop) + offsetY;
